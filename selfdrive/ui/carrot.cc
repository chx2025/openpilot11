...
public:
    ...
    void drawCruiseSpeedBox(const UIState* s) {
        ...
        // 显示定速速度值
        nvgTextAlign(s->vg, NVG_ALIGN_CENTER | NVG_ALIGN_TOP);
        NVGcolor set_speed_color = is_cruise_set ? COLOR_WHITE_ALPHA(200) : COLOR_GREY_ALPHA(200);
        ui_draw_text(s, box_x + box_width / 2, box_y + 77, setSpeedStr, 90, set_speed_color, BOLD);
    }
