...
DevicePanel::DevicePanel(SettingsWindow *parent) : ListWidget(parent) {
  ...
  QPushButton* default_btn = new QPushButton(tr("恢复默认设置"));
  ...
  QPushButton* remove_mapbox_key_btn = new QPushButton(tr("移除 MapboxKey"));
...
SettingsWindow::SettingsWindow(QWidget *parent) : QFrame(parent) {
  ...
  panels.append({ tr("开发人员"), new DeveloperPanel(this) });
...
CarrotPanel::CarrotPanel(QWidget* parent) : QWidget(parent) {
  ...
  startToggles->addItem(new CValueControl("HyundaiCameraSCC", "现代: SCC智能巡航系统", "1:将SCC的CAN线路连接至CAM, 2:同步巡航状态, 3:原厂纵向控制", 0, 3, 1));
  startToggles->addItem(new CValueControl("CanfdHDA2", "CANFD: HDA2 模式", "0:HDA1, 1:HDA2, 2:HDA2+盲点监测BSM", 0, 2, 1));
  startToggles->addItem(new CValueControl("EnableRadarTracks", "启用雷达追踪", "1:启用雷达追踪, -1,2:禁用 (始终使用HKG SCC雷达)", -1, 2, 1));
  startToggles->addItem(new CValueControl("AutoCruiseControl", "自动巡航控制（仅限 HKG 车型）", "1：软保持 + 自动巡航，2：软保持故障时启用", 0, 3, 1));
  startToggles->addItem(new CValueControl("CruiseOnDist", "巡航功能: 自动开启距离(0cm)", "松开踏板且前车靠近时自动开启巡航", 0, 2500, 50));
  startToggles->addItem(new CValueControl("AutoEngage", "启动时自动接管控制", "1:仅启用转向, 2:启用转向 + 巡航待机", 0, 2, 1));
  startToggles->addItem(new CValueControl("AutoGasTokSpeed", "自动加速至设定速度", "油门（加速）至允许速度", 0, 200, 5));
  startToggles->addItem(new CValueControl("SpeedFromPCM", "从PCM读取巡航速度", "丰田必须设为1, 本田设为3", 0, 3, 1));
  ...
  startToggles->addItem(new CValueControl("MaxTimeOffroadMin", "离线自动关机时间 (分钟)", "", 1, 600, 10));
  startToggles->addItem(new CValueControl("EnableConnect", "启用远程连接", "选择连接方式（1：comma，2：carrot）如启用1，克隆设备可能会被 Comma 封禁", 0, 2, 1));
  startToggles->addItem(new CValueControl("MapboxStyle", "地图样式(0)", "0：默认背景，1：深色背景，2：卫星地图", 0, 2, 1));
  startToggles->addItem(new CValueControl("RecordRoadCam", "记录行车录像(0)", "1:仅道路长焦镜头, 2:道路长焦+广角", 0, 2, 1));
  startToggles->addItem(new CValueControl("HDPuse", "使用HDP（CCNC）功能(0)", "1：使用APN时启用 2：始终保持启用", 0, 2, 1));
  ...
  startToggles->addItem(new CValueControl("AutoGasSyncSpeed", "自动更新巡航速度", "若踩下油门且当前车速超过设定值，则以当前车速更新巡航设定速度。", 0, 1, 1));
  startToggles->addItem(new CValueControl("DisableMinSteerSpeed", "禁用最小转向速度限制", "配备 MDPS：1（启用）", 0, 1, 1));
  startToggles->addItem(new CValueControl("DisableDM", "禁用驾驶员监控(DM)", "需重启设备", 0, 1, 1));
  startToggles->addItem(new CValueControl("HotspotOnBoot", "开机启用WIFI热点", "0:不启用,1:启用", 0, 1, 1));
  startToggles->addItem(new CValueControl("SoftwareMenu", "启用软件菜单", "0:不启用,1:启用", 0, 1, 1));
  startToggles->addItem(new CValueControl("IsLdwsCar", "是否LDws车型", "0:不是,1:是", 0, 1, 1));
  ...
  speedToggles->addItem(new CValueControl("AutoCurveSpeedLowerLimit", "自动弯道限速下限(30)", "接近弯道时降低车速, 弯道最低车速设定", 30, 200, 5));
  speedToggles->addItem(new CValueControl("AutoCurveSpeedFactor", "弯道速度系数(100%)", "数值越高，弯道车速越低", 50, 300, 1));
  speedToggles->addItem(new CValueControl("AutoCurveSpeedAggressiveness", "弯道减速力度(100%)", "数值越大，入弯减速越明显", 50, 300, 1));
  speedToggles->addItem(new CValueControl("AutoRoadSpeedLimitOffset", "道路限速偏移量(-1)", "实际限速 = 道路限速 + 设定值。-1：不使用偏移", -1, 100, 1));
  speedToggles->addItem(new CValueControl("AutoRoadSpeedAdjust", "自动调整道路限速(50%)", "-1：设为道路限速，100：采用新道路限速，50：取中间值，0：保持不变", -1, 100, 5));
  speedToggles->addItem(new CValueControl("AutoNaviSpeedCtrlEnd", "测速点前减速完成位置(6秒)", "设置减速完成位置, 数值越大减速越提前完成", 3, 20, 1));
  speedToggles->addItem(new CValueControl("AutoNaviSpeedCtrlMode", "导航限速控制模式(2)", "0:关闭, 1:测速摄像头, 2:+减速带, 3:+移动测速", 0, 3, 1));
  speedToggles->addItem(new CValueControl("AutoNaviSpeedDecelRate", "导航减速率x0.01m/s²(80)", "数值越小, 越早开始减速", 10, 200, 10));
  speedToggles->addItem(new CValueControl("AutoNaviSpeedSafetyFactor", "测速点安全系数(105%)", "在测速摄像头处，将车速降至“道路限速 × 设定比例”", 80, 120, 1));
  ...
  speedToggles->addItem(new CValueControl("AutoNaviCountDownMode", "导航车速控制模式(2)", "0:关闭, 1:仅测速摄像头, 2:测速摄像头+减速带, 2:测速摄像头+减速带+APN测速", 0, 2, 1));
  speedToggles->addItem(new CValueControl("TurnSpeedControlMode", "转弯速度控制模式(1)", "0:关闭, 1:仅视觉识别, 2:视觉识别+route信息, 3:始终按route信息", 0, 3, 1));
  speedToggles->addItem(new CValueControl("MapTurnSpeedFactor", "地图转弯速度系数(100)", "数值越小，沿路线行驶时减速越多。", 50, 300, 5));
  speedToggles->addItem(new CValueControl("AutoTurnControl", "ATC：自动转弯控制(0)", "（APN）0：关闭 1：仅变道辅助 2：变道+调速 3：仅调速", 0, 3, 1));
  speedToggles->addItem(new CValueControl("AutoTurnControlSpeedTurn", "ATC: 转弯速度控制(20)", "（APN）0:无, 设定转弯车速", 0, 100, 5));
  speedToggles->addItem(new CValueControl("AutoTurnControlTurnEnd", "ATC：转弯调速剩余时间(6)", "（APN）以“距离=车速×时间”计算，距转弯完成点*秒前结束速度控制。", 0, 30, 1));
  speedToggles->addItem(new CValueControl("AutoTurnMapChange", "ATC 自动地图显示(0)", "（APN）启用 ATC 时自动打开地图", 0, 1, 1));
